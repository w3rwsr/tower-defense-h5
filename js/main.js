...
(function () {
   ...
   * 信号说明（W3C 规范）：
   * - γ：绕设备长轴（Y 轴）的旋转角
   *   · 垂直横置 ≈±90°（γ<0 听筒朝左，γ>0 听筒朝右）
   *   · 完全平放 ≈0°
   *   · 横置后仰角 0~90° 全程 |γ|≈仰角——低阈值即可覆盖整个倾斜范围
   * - β：绕设备前后轴（X 轴）的旋转角
   *   · 平放屏幕朝上 ≈0°
   *   · 竖持屏幕朝向用户 ≈90°
   *   · 横置（不论听筒朝左/右）≈0°——横置与平放在 β 上相同
   *
   * 状态机（解决"平放横屏时提示层立即遮住游戏"问题）：
   * - 进入横置：|γ|>12°（确保用户主动横置）
   * - 保持横置：|γ|>=8°，或 |γ|<8° 且 β<=45°（疑似平放横屏）
   *   平放横屏时 γ≈0 与竖持在 γ 上无法区分，但 β 可以区分两者：
   *     平放 β≈0（保持横置），竖持 β≈90（退出横置）
   * - 退出横置：|γ|<8° 且 β>45°（明显竖持），或传感器失效
   *
   * 方向锁存：γ 符号仅在 |γ|>10° 时可信（近水平时噪声大），
   *   方向确认后锁存，小倾角沿用上次方向，避免仰角变化过程中
   *   画面 180° 来回翻转。
   *
   * β 不可靠机型的妥协：
   *   部分微信 X5 WebView 的 β 恒报 0（竖持也 ≈0），此时
   *   β<=45° 永远满足，用户从横置→竖持后无法自动退出。

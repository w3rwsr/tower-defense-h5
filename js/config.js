...
window.TD_CONFIG = {
  ...
  "levels": {
     ...
     * 几何（v20260924i：右下道路简化为「出怪→横移一小段→第一个路口直接上拐→
     *   中上部→左拐一小段→右终点」，原右下方方框环绕路全部删除）：
     *   route 0 上路 = 左上出怪 → (560,80) 传送门 A；原 (560,80)→(560,220)
     *                  →(150,220) 那段 A-D 无用道路已删除（普通怪在 A 即传送、
     *                  BOSS 也走 A→B 传去下路，无需这段路）。
     *   route 1 下路 = 右下出怪 (1000,470) → (840,470)[C 第一个路口上拐] →
     *                  (840,340)[B] → (840,200) → (740,200) 右终点。
     *   route 2 左尾段 = (150,220) 传送门 D 出口 → (150,505) 左终点洞穴。
     * 传送门 2 对跨路线缠绕（单向，JSON 驱动）：
     *   cross1: A(route0,w1 560,80) → B(route1,w2 840,340)，affectBoss:true
     *           （上路 BOSS 随小怪一起传去下路走到右终点）；
     *   cross2: C(route1,w1 840,470) → D(route2,w0 150,220)，affectBoss:false
    ...
    "5": {
      ...
      "build": {
        ...
        /* 第 5 关专属额外火力点（buildHotspots 统一做界内/避路/去重硬校验）：
           覆盖上路（y=80）两侧、两路中带，以及右下方删除方框路后留出的
           大块空旷区域内部（横竖对齐 2×2，距任何道路 >90px） */
        "extraSpots": [
          { "x": 616, "y": 40 },  { "x": 688, "y": 40 },   // 顶部右侧一排（覆盖上路）
          { "x": 688, "y": 120 }, { "x": 688, "y": 184 },  // 右竖排（上路下方）
          { "x": 652, "y": 256 }, { "x": 724, "y": 256 },  // 两路中带一排
          { "x": 610, "y": 335 }, { "x": 700, "y": 335 },  // 空旷区上排
          { "x": 610, "y": 405 }, { "x": 700, "y": 405 }   // 空旷区下排
        ]
      },
      ...
      "paths": [
        ...
        /* route 1：右下侧道路（新设计：出怪后横走一小段，在第一个路口 (840,470)
           直接向上拐，直上到中上部 (840,200)，向左拐一小段到右终点；
           无绕圈、无折返、不接回原路；右下方原方框路全部删除，留出大块空旷区）*/
        [
          { "x": 1000, "y": 470 },
          { "x": 840, "y": 470 },   // w1 = 传送门 C（入口）——第一个路口，向上拐
          { "x": 840, "y": 340 },   // w2 = 传送门 B（出口）
          { "x": 840, "y": 200 },   // 直上到中上部
          { "x": 740, "y": 200 }    // w4 = 右终点洞穴（左拐一小段后即达）
        ],

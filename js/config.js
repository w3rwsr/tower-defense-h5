...
window.TD_CONFIG = {
  ...
  "levels": {
    ...
    /* ---------- 第 4 关：交错迷域（双路线先交叉后交汇 + 右路岔路传送门 + 支线波） ----------
     * 共 4 条路线几何：
     *   route 0（左主线）：屏幕左侧出怪 → 右行 → 下行，在 (230,170) 被右路
     *                     横穿（=交叉点，两路线交叉穿过、互不换路）→ 右行 →
     *                     下行，抵达交汇点 (480,340) 后与右路共线通向底部终点；
     *   route 1（右主线）：屏幕右侧出怪 → 左行至岔路口 (720,170)（主路在此
     *                     左转、岔路 route 2 继续直行向下）→ 长横段左行，在
     *                     (230,170) 穿过左路（先交叉）→ 下行 → 右行并入交汇点
     *                     (480,340)（后交汇）→ 与左路合并沿 x=480 下行到终点；
    ...
    "4": {
      ...
      "paths": [
        /* route 0：左主线（左出怪 → 交叉点 → 交汇点 → 底部终点） */
        [
          { "x": -40, "y": 80 },
          { "x": 230, "y": 80 },
          { "x": 230, "y": 260 },    // 竖段在 (230,170) 被 route1 横穿 = 交叉点
          { "x": 480, "y": 260 },
          { "x": 480, "y": 580 }     // 途经交汇点 (480,340)，之后与右路共线
        ],
        /* route 1：右主线（右出怪 → 岔路口 → 交叉点 → 交汇点 → 底部终点） */
        [
          { "x": 1000, "y": 80 },
          { "x": 720, "y": 80 },
          { "x": 720, "y": 170 },    // 岔路口：主路左转，岔路(route2)继续下行
          { "x": 150, "y": 170 },    // 长横段在 (230,170) 穿过 route0 = 交叉点
          { "x": 150, "y": 340 },
          { "x": 480, "y": 340 },    // 交汇点：并入 x=480 共线段
          { "x": 480, "y": 580 }
        ],

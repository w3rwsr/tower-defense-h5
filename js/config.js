...
window.TD_CONFIG = {
  ...
  /* ---------- 放塔规则（沿路双排热区 + 离路校验） ----------
   * 热区不再铺满全图：由路径几何直接生成——沿路径每 cell 采一个点，
   * 向两侧法向各偏移 roadGap，形成【沿道路两侧均匀分布的两排整齐点】；
   * 每个点距路心恒为 roadGap(< 最短射程 125)，保证放塔必能打到路面。
   * 拐弯内侧过近的点按 minSpacing 去重；出界/压路点直接剔除。 */
  "build": {
    "cell": 80,              // 沿路径的采样间距（也是同侧相邻放置点间距）
    "roadGap": 62,           // 放置点距路径中心线的垂直距离（两侧各一排）

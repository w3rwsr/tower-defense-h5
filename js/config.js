...
window.TD_CONFIG = {
  ...
  "levels": {
    ...
    /* ---------- 第 5 关：迷域回廊（最终关：左右对开 + 中间岔路汇合 + 三阶 BOSS） ----------
     * 几何（v20260924j：整体重构为「左右对开 + 中间岔路汇合」）：
     *   route 0 左路 = 左侧出怪 (-40,300) 向右 → (250,300) 岔路口 → 向上岔路至
     *                  (250,140) 岔路顶传送门 → 折回 (250,300) → (380,300)
     *                  传送门出口（主路中段）→ (500,300) 中央汇合 → (500,515)。
     *   route 1 右路 = 右侧出怪 (1040,300) 向左，镜像对称（岔路口 750、岔路顶
     *                  (750,140)、出口 (620,300)、汇合 (500,300)、终点 (500,515)）。
     *   两路在 (500,300) 汇合后共用同一竖向道路，通向下方唯一共享终点洞穴。
     * 传送门 2 对（单向，同色配对、无字母）：
     *   forkL: 岔路顶(250,140) → 主路中段(380,300)，affectBoss:false
     *          （小怪到岔路顶即被传回主路中段；BOSS 不传送，走完整路线含岔路
     *           上+折回下，全程 ~1075px，三阶段击杀窗口充足）；
     *   forkR: 岔路顶(750,140) → 主路中段(620,300)，镜像同上。
     * 共享终点：goals 两条路线同指 (500,515) 洞穴，任一边漏怪扣同一条命。
     * 波次：常规 5 波 spawnRoutes=[0,1]（左右同时出怪）；hpGrowth=1.6；
    ...
    "5": {
      ...
      "build": {
        ...
        /* 第 5 关专属额外火力点（buildHotspots 统一做界内/避路/去重硬校验）：
           自动点只贴路 62px，这里补足两岔路之间的上部空旷区纵深、
           汇合竖路下段两侧，保证主路/岔路/汇合区全被火力覆盖 */
        "extraSpots": [
          { "x": 400, "y": 110 }, { "x": 500, "y": 110 }, { "x": 600, "y": 110 },  // 顶部内区一排
          { "x": 420, "y": 175 }, { "x": 480, "y": 175 }, { "x": 580, "y": 175 },  // 岔路间中带一排
          { "x": 360, "y": 470 }, { "x": 640, "y": 470 }                            // 汇合竖路下段两侧
        ]
      },
      "path": { "borderColor": 0xc99a54, "fillColor": 0xeac58f },
      "paths": [
        /* route 0：左路（左侧出怪向右 → 岔路口上拐 → 岔路顶传送门 → 折回 →
           传送门出口（主路中段）→ 中央汇合 → 下方共享终点。
           小怪在岔路顶即被传回主路中段，BOSS 走完整路线含岔路折返） */
        [
          { "x": -40, "y": 300 },
          { "x": 250, "y": 300 },   // w1 = 岔路口（向上开岔）
          { "x": 250, "y": 140 },   // w2 = 岔路顶 = 传送门入口（forkL）
          { "x": 250, "y": 300 },   // w3 = 折回岔路口（仅不传送的 BOSS 走）
          { "x": 380, "y": 300 },   // w4 = 传送门出口（主路中段）
          { "x": 500, "y": 300 },   // w5 = 中央汇合点
          { "x": 500, "y": 515 }    // w6 = 共享终点洞穴（地图下方）
        ],
        ...
        [
          ...
          { "x": 620, "y": 300 },   // w4 = 传送门出口（主路中段）
          { "x": 500, "y": 300 },   // w5 = 中央汇合点
          { "x": 500, "y": 515 }    // w6 = 共享终点洞穴
        ]
      ],
      /* 共享终点（JSON 驱动）：两条路线末路径点同为 (500,515)，洞穴重叠绘制
         为同一个；任一路线漏怪都扣同一条命（HUD 单生命显示） */
      "goals": [
        { "route": 0, "label": "共享终点" },
        { "route": 1 }
      ],
      /* 传送门 2 对（岔路顶 → 主路中段，单向，同色配对、无字母）；
